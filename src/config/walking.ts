 ...
 */
export function onKeyDownWalking(
    event: KeyboardEvent,
    keyManager: InputManager,
) {
...
function mutateLegacyFlags(down: boolean, code: string, km: any) {
    switch (code) {
        ...
        case 'ArrowRight':
            ...
            break
        case 'Space':
            if (down && km.canJump) km.jumpPressed = true
            break
    }
}
...
export function addObstacle(
    ...
    source: THREE.Object3D | THREE.Box3,
    scene: THREE.Scene,
) {
    ...
    if (source instanceof THREE.Box3) {
        ...
        console.log(
            `[Obstacle Created] Name: ${source.name}, Min: ${box.min.x.toFixed(2)}, ${box.min.y.toFixed(2)}, ${box.min.z.toFixed(2)}, Max: ${box.max.x.toFixed(2)}, ${box.max.y.toFixed(2)}, ${box.max.z.toFixed(2)}`,
        )
...
export function makeCollisionManager(
    ...
    const debugArrow = opts.debugRay ? new THREE.ArrowHelper() : undefined
...
function simpleBoxClamp(
    yawObject: { position: { x: any; z: any } },
    obstacleBoxes: any,
) {
    // ———————————————— simple box clamp ————————————————
    // snap to the highest static box under your feet
    const px = yawObject.position.x,
        pz = yawObject.position.z
    let bestY = -Infinity
...
export function walkingAnimationCallback(
    scene: THREE.Scene,
    controls: any,
    _: any,
    elapsed: number,
    ...
    const collision =
        controls?.threejsDrawing?.data?.collision || controls?.collision
    if (!collision) return

    if (
        controls.isLocked === true ||
        (override === true && controls.name === 'PointerLockControls')
    ) {
