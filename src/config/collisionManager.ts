...
export function getYawObject(controls: any): THREE.Object3D {
    const yawObject = controls.object?.quaternion
        ? controls.object // camera
        : controls?._controls?.object?.quaternion
          ? controls._controls.object
          : controls
...
export function extractPerTriangle(
...
export function checkCollisionSpatialHashes(
    ...
    tempBox.setFromCenterAndSize(
        position,
        new THREE.Vector3(playerSize, playerSize * 2, playerSize),
    )
...
export class CollisionSystem {
    ...
    slideHorizontal(p: THREE.Object3D, v: THREE.Vector3, dt: number) {
        if (!this._collides(this._nextPos(p, v.x, 0, dt)))
            p.position.x += v.x * dt
        if (!this._collides(this._nextPos(p, 0, v.z, dt)))
            p.position.z += v.z * dt
    }
    ...
    private _nextPos(p: THREE.Object3D, dx = 0, dz = 0, dt = 0) {
        return p.position
            .clone()
            .addScaledVector(new THREE.Vector3(dx, 0, dz), dt)
    }
    ...
    private _collides(pt: THREE.Vector3): boolean {
        const playerBox = new THREE.Box3().setFromCenterAndSize(
            pt,
            new THREE.Vector3(
                this.cfg.PLAYER_SIZE,
                this.cfg.PLAYER_HEIGHT,
                this.cfg.PLAYER_SIZE,
            ),
        )
    ...
    private _firstWalkableHit(p: THREE.Object3D, meshes: THREE.Mesh[]) {
        return this.ray
            .intersectObjects(this._rayTargets(p, meshes), true)
...
export class CollisionManager {
    ...
    update(controls?: any, dt?: number, ignore: any = null) {
        ...
        const cameraDir = getCameraRelativeDirection(
            yawObject,
        ...
        if (
            ...
            const angle =
                (this.input.keys['ArrowLeft'] ? -1 : 1) * turnSpeed * delta
