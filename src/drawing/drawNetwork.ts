...
function renderD3Tree(
    scene: THREE.Scene,
    nodes2D: HierarchyPointNode<any>[],
    links2D: {
...
function computeDepths(graph: Graph, rootId: number | string) {
    ...
    const outgoing = graph.links.reduce(
        (m: { [key: string]: any[] }, l) => {
    ...
    graph.nodes.forEach((n) => {
        n.depth = depthMap[n.id] ?? Infinity
    })
}
 ...
 */
function updateThreeJSPositions(
    nodeMeshes: THREE.Mesh[],
    linkMeshes: THREE.Mesh[],
    graph: Graph,
) {
    ...
    linkMeshes.forEach((mesh, i) => {
        ...
        const src =
            typeof source === 'object'
                ? source
                : graph.nodes.find((n) => n.id === source)
        const tgt =
            typeof target === 'object'
                ? target
                : graph.nodes.find((n) => n.id === target)
...
function applyForce(
    ...
    const simulation = forceSimulation(graph.nodes)
        ...
        .force(
            'treeX',
            forceX(0)
                .x((d: GraphNode) => (d.id === ROOT_ID ? 0 : d.depth * SPACING))
                .strength(1),
        )
...
function connectTreeNodes(
    ...
    for (const { source, target, color = 'orange' } of rawLinks) {
        ...
        const isInTree = treeLinks.some(
            (l) => l.source.data.id === source && l.target.data.id === target,
        )
...
function drawTreeWithExtras(
    ...
    const { dx = 30, dy = 60, floorY = 0.5 } = options
    ...
    const laidOutNodes: HierarchyPointNode<any>[] =
        rootNode.descendants() as HierarchyPointNode<any>[]
...
function handleUp(
    ...
    if (!navState.current) {
        ...
        const root = [...nodeMap.values()].reduce((a, b) =>
            a.data.id < b.data.id ? a : b,
        )
...
function handleLeft(
    ...
    if (neighbors.length > 1) {
        navState.selectionIndex =
            (navState.selectionIndex - 1 + neighbors.length) % neighbors.length
...
function handleSideways(
    ...
    if (nextNode) {
        ...
        if (navState.current) {
            const xyz2 = {
                x: navState.current.x ?? 0,
                y: navState.current.y ?? 0.5,
                z: navState.current.y ?? 0,
            }
...
const networkDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawNetwork, dataSrc: 'force', dataType: 'json' }],
