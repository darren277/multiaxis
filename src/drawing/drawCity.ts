...
const STREETS = ['Object_4', 'Object_5', 'Object_6', 'Object_7']
...
const OBSTACLES = ['Object_59', 'Object_124', 'Object_205']
    ...
    loadGltfModel('city/scene').then((gltf) => {
        ...
        model.traverse((child: THREE.Object3D) => {
            if (child instanceof THREE.Mesh) {
                ...
                if (
                    STREETS.includes(child.name) ||
                    SIDEWALKS.includes(child.name)
                ) {
        ...
        console.log(
            'streets found:',
            threejsDrawing.data.worldMeshes?.length ?? 0,
        )
    })
...
const cityDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawCity, dataSrc: null, dataType: 'gltf' }],
    eventListeners: {
    ...
    animationCallback: (
        ...
        if (!threejsDrawing.data.spawned) {
            console.log(
                'teleporting to (for realsies):',
                threejsDrawing.data.spawnCoords,
            )
