...
function updateLeaderLine(
    ...
    endVector.set(
        (parseFloat(labelElement.style.left) / canvas.clientWidth) * 2 - 1,
        -(parseFloat(labelElement.style.top) / canvas.clientHeight) * 2 + 1,
        0.5,
    )
...
const brainDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawBrain, dataSrc: null, dataType: 'ply' }],
    eventListeners: null,
    animationCallback: (
