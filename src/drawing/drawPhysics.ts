...
function drawTorqueAndAngularMotion(scene: THREE.Scene, physicsWorld: any) {
    ...
    const cylinder = createRigidBody(
        'Cylinder',
        physicsWorld,
        shape,
        3,
...
function drawFriction(scene: THREE.Scene, physicsWorld: any) {
    ...
    // A sliding box
    const boxShape = new Ammo.btBoxShape(new Ammo.btVector3(1, 1, 1))
    const slidingBox = createRigidBody(
        'Sliding Box',
        physicsWorld,
        boxShape,
        2,
...
function drawElasticForce(scene: THREE.Scene, physicsWorld: any) {
    ...
    const spring = new Ammo.btPoint2PointConstraint(
        pointA.body,
        pointB.body,
        pivotA,
        pivotB,
...
function drawGUI(box: any) {
    ...
    physicsFolder
        .add(params, 'mass', 0.1, 10)
        .onChange((newMass: number) => updateMass(box, newMass))
    physicsFolder
        .add(params, 'friction', 0, 1)
        .onChange((val: number) => updateFriction(box, val))
    physicsFolder
        .add(params, 'restitution', 0, 1)
        .onChange((val: number) => box.body.setRestitution(val))
...
function drawPhysics(scene: THREE.Scene, threejsDrawing: any) {
    ...
    Ammo().then(function (AmmoLib: any) {
        ...
        initPhysics(threejsDrawing).then((physicsWorld) => {
            ...
            const rigidNewtonianBodies = drawBasicNewtonianForces(
                scene,
                physicsWorld,
            )
...
const physicsDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawPhysics, dataSrc: null }],
    eventListeners: {},
    animationCallback: (
