...
function layoutFamilyWithSpread(graph: FamilyGraph, rootId: number) {
    ...
    while (q.length) {
        ...
        parents.forEach((pid: number, idx: number) => {
            ...
            p.x =
                branchDir * (depth + 1) * X_STEP + // main offset
...
function layoutWithD3(graph: FamilyGraph, rootId: number) {
    ...
    d3
        .tree<MyHierarchyNode>()
        .nodeSize([X_STEP, GEN_STEP])
        ...
        function drawLabel() {
            ...
            for (const w of words) {
                ...
                if (ctx.measureText(test).width <= maxTextW) {
                    ...
                    ctx.fillText(
                        line,
                        (side - ctx.measureText(line).width) / 2,
                        yPos,
                    )
                    line = w
                    yPos += fontSize + 2
...
async function drawFamilyTree(
    ...
    graph.nodes.forEach(async (n) => {
        ...
        const sprite = (await labelSpriteWithImage(
            n.name,
            n.imageUrl || null,
        )) as THREE.Sprite
