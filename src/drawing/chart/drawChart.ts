    ...
    loader.load(config.fontUrl, (font: Font) => {
        ...
        graphData.points.forEach((point: any) => {
            ...
            if (icon) {
                ...
                const pointMaterial = config.points.material(
                    point,
...
const multiAxisDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawChart, dataSrc: 'data', dataType: 'json' }],
    eventListeners: {
        ...
        click: (event: MouseEvent, context: any) => {
            ...
            const intersects = raycaster.intersectObjects(
                context.data.interactiveObjects,
            )
            ...
            if (intersects.length > 0) {
                ...
                if (clickedObject.userData.htmlContent) {
                    console.log(
                        'Clicked on object with HTML content:',
                        clickedObject.userData.htmlContent,
                    )
    ...
    animationCallback: (
        ...
        raycaster.setFromCamera(mouse, camera)
        const intersects = raycaster.intersectObjects(
            threejsDrawing.data.interactiveObjects,
        )
