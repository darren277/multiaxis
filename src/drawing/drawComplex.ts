...
import {
    drawTV,
    animationCallback as tvAnimation,
    onClick as tvOnClick,
} from './reusables/drawTV'
...
function drawComplex(scene: THREE.Scene, threejsDrawing: ThreeJSDrawing) {
    if (!threejsDrawing.uniqueId) {
        ...
        console.log(
            `%c[LIFECYCLE] Main threejsDrawing object tagged with ID: ${threejsDrawing.uniqueId}`,
    ...
    // Draw the screen
    const css3DRenderer = threejsDrawing.data.css3DRenderer as {
        scene: THREE.Scene
    }
    const {
        mesh: screenMesh,
...
const complexDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawComplex, dataSrc: null }],
    eventListeners: {
        click: (event: MouseEvent, data: any) => {
            ...
            const wasTvClickHandled = tvOnClick(
                event,
                data.scene,
                data.camera,
                data.renderer,
