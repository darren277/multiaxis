...
export function casePitchX(lib: Library) {
    // centre‑to‑centre distance in X
    return lib.bookcase.width + lib.spaceBetweenCases
}

// Distance between the centres of two neighbouring rows (z direction).
export function rowPitchZ(lib: Library) {
    // centre‑to‑centre distance in Z
    return lib.bookcase.depth + lib.spaceBetweenRows
}

export function worldX(row1StartX: number, lib: Library, caseIdx: number) {
    return row1StartX + caseIdx * casePitchX(lib)
}

export function worldZ(row0StartZ: number, lib: Library, rowIdx: number) {
    return row0StartZ + rowIdx * rowPitchZ(lib)
}

// Convert  (row, case, shelf)  →  world‑space coordinate that matches the
// positions produced in createBookCases().
function shelfToWorld(
    row1StartX: number,
    row0StartZ: number,
    library: Library,
    rowIdx: number,
 ...
 */
export function calculatePositionOfResource(
    resource: Resource,
    library: Library,
    row1StartX: number,
    row0StartZ: number,
    ...
    const totalShelves =
        library.numberOfRows * library.numberOfCases * SHELVES_PER_CASE
