...
async function renderCard(card: any, cfg: any) {
    ...
    const cardBackTexture = textureLoader.load(
        `textures/${card.backTexture || 'cards/back_texture.png'}`,
    )
...
async function generateCardPositions(
    numCards: number,
    horizontalSpacing = 3,
    verticalSpacing = 3,
) {
...
function animateCardSwap(
    ...
    // 1) Lift up
    const liftTween = new TWEEN.Tween(card.position)
        .to({ y: card.position.y + liftHeight }, 300)
        .easing(TWEEN.Easing.Cubic.Out)

    tweenGroup.add(liftTween)

    // 2) Move horizontally
    const moveTween = new TWEEN.Tween(card.position)
        .to({ x: targetPos.x, z: targetPos.z }, 600)
        .easing(TWEEN.Easing.Cubic.InOut)

    tweenGroup.add(moveTween)

    // 3) Drop down
    const dropTween = new TWEEN.Tween(card.position)
        .to({ y: targetPos.y }, 300)
...
function dealOneCard(cards: THREE.Mesh[], cfg: any) {
    ...
    if (cfg.orientation === 'landscape') {
        newRot = {
            x: card.rotation.x + Math.PI / 2,
            z: card.rotation.z + Math.PI / 2 + Math.PI / 2,
    ...
    new TWEEN.Tween(card.position)
        .to({ x: 0, y: 0, z: newZPos }, 1000)
        .easing(TWEEN.Easing.Quadratic.InOut)
        .start()

    // Animate rotation (Y only)
    new TWEEN.Tween(card.rotation)
        .to({ x: newRot.x, y: newRot.y, z: newRot.z }, 1000)
        .easing(TWEEN.Easing.Quadratic.InOut)
        .start()
}
...
function animationCallback(tweenGroup: TWEEN.Group, cards: THREE.Mesh[]) {
    ...
    for (
        let tries = 0;
        tries < maxTries && launched < SWAPS_PER_PRESS;
        tries++
    ) {
...
function drawCards(scene: THREE.Scene, data: any, threejsDrawing: any) {
    ...
    const deckType: 'portrait' | 'landscape' =
        metadata.orientation === 'landscape' ? 'landscape' : 'portrait' // default to portrait

    const cfg = DeckConfig[deckType]

    cfg.anisotropy =
        threejsDrawing.data.renderer.capabilities.getMaxAnisotropy()

    renderCards(scene, cardsArray, cfg)
        .then(async (cardMeshes) => {
            const cardPositions = await generateCardPositions(
                cardsArray.length,
                3,
                4,
            ) // spacing of 3 units
            ...
            window.addEventListener('mousedown', (e) =>
                onMouseDown(e, threejsDrawing),
            )
            window.addEventListener('mousemove', (e) =>
                onMouseMove(e, threejsDrawing),
            )
            window.addEventListener('mouseup', (e) =>
                onMouseUp(e, threejsDrawing),
            )
        })
        .catch((err) => {
...
function logDealtCards() {
    ...
    const dealtCardPositions = window.cardPositions.filter(
        (_: any, i: number) => dealtCardsArray.includes(i),
    )
    console.log(
        'Dealt cards:',
        dealtCardsArray,
...
const cardsDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawCards, dataSrc: 'cards', dataType: 'json' }],
    eventListeners: null,
    animationCallback: (
        ...
        if (isDPressed) {
            ...
            const handValue = calculateHandValue(
                dealtCards,
                threejsDrawing.data.cardsArray as any[],
            )
