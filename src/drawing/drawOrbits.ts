...
function createPlanet(planetData: PlanetData, scene: THREE.Scene): Planet {
    ...
    const radius =
        (planetData.diameter / 2) * SCALE_FACTOR * PLANET_SIZE_SCALE_FACTOR
...
function castStars(
    scene: THREE.Scene,
    threejsDrawing: ThreeJSDrawing & { data: OrbitsDrawingData },
) {
...
const orbitsDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawOrbits, dataSrc: null }],
    eventListeners: null,
    animationCallback: (
