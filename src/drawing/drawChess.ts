...
export function drawChessBoard(
    ...
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            ...
            square.position.x =
                col * squareSize - 4 * squareSize + squareSize / 2
            square.position.z =
                row * squareSize - 4 * squareSize + squareSize / 2
...
function createPlaceholderPiece(
    scene: THREE.Scene,
    type: string,
    color = 0xeeeeee,
    font: Font | null = null,
...
function drawChess(scene: THREE.Scene, threejsDrawing: ThreeJSDrawing) {
    const fontLoader = new FontLoader()
    fontLoader.load(
        'scripts/threejs/helvetiker_regular.typeface.json',
        (font) => {
...
const chessDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawChess, dataSrc: null }],
    eventListeners: null,
    animationCallback: (
