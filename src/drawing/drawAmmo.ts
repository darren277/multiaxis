...
function createObject(scene: THREE.Scene, physicsWorld: any, mass: number, halfExtents: THREE.Vector3, pos: THREE.Vector3, quat: THREE.Quaternion, material: THREE.Material) {
...
function createObjects(scene: THREE.Scene, physicsWorld: any) {
    ...
    pos.set(8, 5, 0);
    quat.set(0, 0, 0, 1);
    createObject(scene, physicsWorld, towerMass, towerHalfExtents, pos, quat, createMaterial(0xB03214));
    ...
    for (let i = 0; i < numStones; i ++) {
        ...
        createObject(scene, physicsWorld, stoneMass, stoneHalfExtents, pos, quat, createMaterial(0xB0B0B0));
...
function createParalellepipedWithPhysics(scene: THREE.Scene, physicsWorld: Ammo.btDiscreteDynamicsWorld, sx: number, sy: number, sz: number, mass: number, pos: THREE.Vector3, quat: THREE.Quaternion, material: THREE.Material) {
    ...
    createRigidBody(scene, physicsWorld, object, shape, mass, pos, quat, null, null);
...
const ammoDrawing = {
    ...
    'animationCallback': (renderer: THREE.WebGLRenderer, timestamp: number, threejsDrawing: any, camera: THREE.Camera) => {
        ...
        updatePhysics(scene, physicsWorld, dispatcher, deltaTime, numObjectsToRemove);
    },
    'data': {
    },
    'sceneConfig': {
