...
function updateForceGraph(
    graphData: any,
    nodeSpheres: { [key: string]: THREE.Mesh },
    linkLines: { line: THREE.Line; link: any }[],
) {
    ...
    for (const { line, link } of linkLines) {
        const src = graphData.nodes.find(
            (n: any) => n.id === link.source.id || n.id === link.source,
        )
        const tgt = graphData.nodes.find(
            (n: any) => n.id === link.target.id || n.id === link.target,
        )
...
function onMouseDown(
    ...
    if (intersects.length > 0) {
        ...
        const foundEntry = Object.entries(data.nodeSpheres).find(
            ([id, obj]) => obj === mesh,
        )
        const nodeId = foundEntry ? foundEntry[0] : null

        data.draggedNode = nodeId
            ? data.graphData.nodes.find((n: { id: string }) => n.id == nodeId)
            : null
    }
}
...
function drawForceGraph(
    ...
    const { simulation, nodeSpheres, linkLines } = drawForceDirectedGraph(
        scene,
        gData,
    )
...
const cayleyDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawGraph, dataSrc: 'cayley', dataType: 'json' }],
    eventListeners: null,
    animationCallback: (
