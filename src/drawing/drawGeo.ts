...
async function loadAndRenderGeoJSON(
    ...
    geojson.features.forEach((feature: any) => {
        if (feature.geometry.type === 'Polygon') {
            ...
            const isBuilding =
                tags.building &&
                (tags.building === 'yes' || tags.building === 'house')
...
function drawBuildings(scene: THREE.Scene, threejsDrawing: ThreeJSDrawing) {
    ...
    drawMap(scene, threejsDrawing)
        .then(() => {
            loadAndRenderGeoJSON(scene, buildingGroupParams)
                .then(() => {
...
function createGeoJsonMap(
    ...
    geojson.features.forEach((feature: any) => {
        ...
        if (geometryType === 'Polygon') {
            ...
            const polygonGroup = polygonToMesh3D(
                scene,
                coords,
                projectionFn,
                debug,
...
function drawGeo(scene: THREE.Scene, threejsDrawing: ThreeJSDrawing) {
    const projectionFn = (lng: number, lat: number) => {
        return equirectangularProjection(lng, lat)
    }
...
const buildingsDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawBuildings, dataSrc: null }],
    eventListeners: null,
    animationCallback: (
        renderer: THREE.WebGLRenderer,
        timestamp: number,
