    ...
    return [
        new THREE.MeshBasicMaterial({
            color: faceColors.right
                ? colors[faceColors.right as ColorKey]
                : colors.black,
        }), // +X
        new THREE.MeshBasicMaterial({
            color: faceColors.left
                ? colors[faceColors.left as ColorKey]
                : colors.black,
        }), // -X
        new THREE.MeshBasicMaterial({
            color: faceColors.top
                ? colors[faceColors.top as ColorKey]
                : colors.black,
        }), // +Y
        new THREE.MeshBasicMaterial({
            color: faceColors.bottom
                ? colors[faceColors.bottom as ColorKey]
                : colors.black,
        }), // -Y
        new THREE.MeshBasicMaterial({
            color: faceColors.front
                ? colors[faceColors.front as ColorKey]
                : colors.black,
        }), // +Z
        new THREE.MeshBasicMaterial({
            color: faceColors.back
                ? colors[faceColors.back as ColorKey]
                : colors.black,
        }), // -Z
    ]
}
...
function getFaceCubelets(
    ...
    return cubelets.filter(
        (cubelet) => Math.abs(cubelet.position[axis] - value) < epsilon,
    )
}
...
const rubiksCubeDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawRubiksCube, dataSrc: null }],
