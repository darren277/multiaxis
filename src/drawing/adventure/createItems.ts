...
function create3DLabelWithAnimation(
    captionText: string,
    className: string,
...
function createCaptionedItem(
    scene: THREE.Scene,
    item: Item,
    isVideo: boolean,
    worldWidth: number | null = null,
    worldHeight: number | null = null,
    use3dRenderer: boolean = false,
) {
    let mesh = null
    ...
    if ((item.image || item.video) && item.image !== 'NO_IMAGE') {
        if (isVideo) {
            if (!worldWidth || !worldHeight) {
                console.warn(
                    'World dimensions not provided for video mesh creation.',
                )
                return null // or handle this case as needed
            }
    ...
    if (use3dRenderer) {
        ...
        const labelObject = create3DLabelWithAnimation(
            captionText,
            customClasses,
            dataAttributes,
        )
