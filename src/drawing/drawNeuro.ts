...
function createNeurotransmitter(
    type: NeurotransmitterType = 'glutamate',
...
function drawSynapse(scene: THREE.Scene, threejsDrawing: ThreeJSDrawing) {
    ...
    const preMaterial = new THREE.MeshStandardMaterial({
        color: 0x4444ff,
        transparent: true,
        opacity: 0.6,
    })
    ...
    receptorTypes.forEach((type, i) => {
        for (let j = 0; j < 4; j++) {
            const receptor = createReceptor(
                type as ReceptorType,
                new THREE.Vector3(
                    -1.5 + j * 1.0,
                    -4,
                    (Math.random() - 0.5) * 2,
...
function releaseNeurotransmitters(
    ...
    vesicles.forEach((v) => {
        const type = transmitters[
            Math.floor(Math.random() * transmitters.length)
...
function resetReceptors(receptorMeshes: THREE.Mesh[]) {
    receptorMeshes.forEach((r) => {
        ...
        ;(r.material as THREE.MeshStandardMaterial).color.set(
            modelRegistry.receptors.materials[r.userData.type as ReceptorType]
                .color,
        )
    })
}
...
function animateSynapse(
    ...
    receptorMeshes.forEach((r) => {
        if (r.userData.pulse > 0) {
            ...
            ;(r.material as THREE.MeshStandardMaterial).color.set(
                receptorColorMap[r.userData.type as ReceptorType],
            )
        }
    })
...
const synapseDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawSynapse, dataSrc: null, dataType: null }],
    eventListeners: null,
    animationCallback: (
        ...
        const {
            ...
            vesicles = [],
            receptors = [],
        } = threejsDrawing.data as SynapseDrawingData
