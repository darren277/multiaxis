...
function isGiantWhiteBox(path: Path) {
    ...
    const isGiantWhiteBox =
        ...
        path.toShapes(true).length === 1
    if (isGiantWhiteBox) {
        console.log('Giant white box detected')
        console.log(
            'isGiantWhiteBox',
            fillColor,
            path.toShapes(true).length,
            isGiantWhiteBox,
        )
    }
...
function processPath(path: any) {
    ...
    shapes.forEach((shape: THREE.Shape) => {
        ...
        if (fillColor === 'none' && isText) {
            console.warn(
                'Skipping shape with fillColor none or linear gradient',
                fillColor,
                path,
            )
            theColor = 'black'

            const mesh = processShape(
                shape,
                6,
                theColor,
                isText,
...
function buildSvgGroup({
    ...
    group.rotation.fromArray(
        [...rotation, 0, 0, 0].slice(0, 3) as [number, number, number],
    )
...
function pathToMeshes(path: any, fallbackDepth: number = defaultDepth) {
    ...
    const { depth, fillColor } = deriveColorAndDepth(
        type,
        fill,
        style.stroke,
        fallbackDepth,
    ...
    return shapes.map((shape: THREE.Shape) =>
        processShape(shape, depth, fillColor, isText),
    )
}
...
async function drawMultipleSvgs(
    scene: THREE.Scene,
    data: any,
    threejsDrawing: ThreeJSDrawing,
) {
        ...
        if (hovered)
            hovered.traverse((ch) => {
                if (
                    ...
                    ((ch as THREE.Mesh).material as any).color
                ) {
                    ;((ch as THREE.Mesh).material as any).color.copy(
                        ch.userData.origColor,
                    )
                }
            })
        hovered = newHovered
            ...
            hovered.traverse((ch) => {
                if (
                    ...
                    if (!ch.userData.origColor) {
                        ch.userData.origColor = (
                            (ch as THREE.Mesh).material as any
                        ).color.clone()
                    }
                    ;((ch as THREE.Mesh).material as any).color.offsetHSL(
                        0,
                        0,
                        0.2,
                    )
...
const svgDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawSvg, dataSrc: 'OpenProject', dataType: 'svg' }],
    eventListeners: null,
    animationCallback: (
        renderer: THREE.WebGLRenderer,
        timestamp: number,
...
const multiSvgDrawing = {
    sceneElements: [],
    drawFuncs: [{ func: drawMultipleSvgs, dataSrc: null, dataType: 'svg' }],
    eventListeners: {
...
function deriveColorAndDepth(
    type: any,
    fill: any,
    stroke: any,
    fallbackDepth: number,
): { depth: any; fillColor: any } {
