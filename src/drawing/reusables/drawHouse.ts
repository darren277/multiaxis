...
function addDoorAndWindow(
    ...
    const frontWall = houseGroup.children.find(
        (obj) =>
            obj instanceof THREE.Mesh &&
            obj.geometry instanceof THREE.BoxGeometry &&
            Math.abs(obj.position.z - (wallDepth / 2 - wallThickness / 2)) <
                0.01,
    )
    ...
    const door = new THREE.Mesh(
        new THREE.BoxGeometry(doorWidth, doorHeight, wallThickness * 0.9),
        doorMat,
    )
    door.position.set(
