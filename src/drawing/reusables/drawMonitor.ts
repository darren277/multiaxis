...
const sources = [
    {
        ...
        type: 'gltfModel',
        path: 'imagery/computer_setup.glb',
    },
    {
        name: 'computerSetupTexture',
        type: 'texture',
        path: 'textures/monitor/baked_computer.jpg',
    },
    {
        name: 'monitorSmudgeTexture',
        type: 'texture',
        path: 'textures/monitor/smudges.jpg',
    },
    {
        name: 'monitorShadowTexture',
        type: 'texture',
        path: 'textures/monitor/shadow-compressed.png',
    },
]
...
export default class MonitorScreen extends EventEmitter {
            ...
            switch (src.type) {
                case 'gltfModel':
                    return gltfLoader
                        ...
                        .then(
                            (file: GLTF) =>
                                (this.items.gltfModel[src.name] = file),
                        )
                ...
                case 'cubeTexture':
                    ...
                    return cubeTextureLoader
                        .loadAsync(
                            Array.isArray(src.path) ? src.path : [src.path],
                        )
                        .then(
                            (file: THREE.CubeTexture) =>
                                (this.items.cubeTexture[src.name] = file),
                        )
                ...
                case 'audio':
                    return audioLoader
                        ...
                        .then(
                            (buffer: AudioBuffer) =>
                                (this.items.audio[src.name] = buffer),
                        )
    ...
    initializeScreenEvents() {
        document.addEventListener(
            ...
            (event: MouseEvent) => {
                const id =
                    event.target && 'id' in event.target
                        ? (event.target as HTMLElement).id
                        : null
                if (id === 'computer-screen') {
                    ;(
                        event as MouseEvent & { inComputer?: boolean }
                    ).inComputer = true
                }
    ...
    update() {
        if (this.dimmingPlane && this.camera && this.position) {
            ...
            ;(this.dimmingPlane.material as THREE.MeshBasicMaterial).opacity =
...
async function attachSceneComponentsToMonitor(
    monitor: any,
...
async function drawMonitor(scene: THREE.Scene, threejsDrawing: ThreeJSDrawing) {
    ...
    if (
        !threejsDrawing.data.cssScene ||
        !(threejsDrawing.data.cssScene instanceof THREE.Scene)
    ) {
    ...
    await attachSceneComponentsToMonitor(
        data.monitorScreen,
        threejsDrawing,
        cssScene,
    )
    ...
    const computer = new Computer(
        scene,
        data.monitorScreen.items.gltfModel,
        data.monitorScreen.items.texture,
    )
    ...
    if (
        data.monitorScreen.camera &&
        data.monitorScreen.position &&
        data.monitorScreen.rotation
    ) {
...
function animationCallback(
    ...
    const cssRenderer = threejsDrawing.data
        .cssRenderer as import('three/examples/jsm/renderers/CSS3DRenderer').CSS3DRenderer
    ...
    if (!cssScene || !(cssScene instanceof THREE.Scene)) {
        console.warn(
            'CSS Scene not found or not a THREE.Scene, creating a new THREE.Scene.',
        )
