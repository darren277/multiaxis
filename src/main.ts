...
export function loadDrawingName() {
    const drawingNameMeta = document.querySelector(
        'meta[name="threejs_drawing_name"]',
    )
    if (!drawingNameMeta) {
...
export async function onContentLoaded() {
    ...
    try {
        const drawingLoader = (
            THREEJS_DRAWINGS as unknown as ThreeJSDrawingsMap
        )[drawingName] as unknown as () => Promise<ThreeJSDrawing>
        if (!drawingLoader) {
        ...
        if (typeof drawingLoader !== 'function') {
            console.error(
                `Drawing loader for ${drawingName} is not a function.`,
            )
            return
        }
        ...
        if (Flags.includeLocal) {
            ...
            console.log(
                '[TEST DEBUG] in fallback branch, Flags.includeLocal=',
                Flags.includeLocal,
            )
            ...
            const localDrawing: (() => Promise<ThreeJSDrawing>) | undefined = (
                localMap as unknown as Record<
                    string,
                    () => Promise<ThreeJSDrawing>
                >
            )[drawingName]
            if (!localDrawing) {
            ...
            console.error(
                `No drawing found for ${drawingName} and INCLUDE_LOCAL is false.`,
            )
        }
    }
}
